...
func init() {
	...
	govalidator.AddCustomRule("not_exists", func(field string, rule string, message string, value interface{}) error {
		...
		// 拼接 SQL
		query := database.Gohub_DB.Table(tableName).Where(dbFiled+" = ? && isnull(deleted_at)", requestValue)
	...
	govalidator.AddCustomRule("exists", func(field string, rule string,
		...
		// 查询数据库
		var count int64
		database.Gohub_DB.Table(tableName).Where(dbFiled+"= ?", requestValue).Count(&count)
		// 验证不通过，数据不存在
		if count == 0 {
			// 如果有自定义错误消息的话，使用自定义消息
			if message != "" {
