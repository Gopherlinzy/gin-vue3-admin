...
func NewMigrator() *Migrator {
	...
	migrator := &Migrator{
		...
		DB:       database.Gohub_DB,
		Migrator: database.Gohub_DB.Migrator(),
	}
...
func (migrator *Migrator) rollbackMigrations(migrations []Migration) bool {
	...
	for _, _migration := range migrations {
		...
		if mfile.Down != nil {
			mfile.Down(database.Gohub_DB.Migrator(), database.SqlDB)
		}
...
func (migrator *Migrator) runUpMigration(mfile MigrationFile, batch int) {
	...
	if migrator.Up != nil {
		...
		// 执行 up 方法
		mfile.Up(database.Gohub_DB.Migrator(), database.SqlDB)
		// 提示已迁移了哪个文件
