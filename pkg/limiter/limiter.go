...
func CheckRate(c *gin.Context, key string, formatted string) (limiterlib.Context, error) {
	...
	store, err := sredis.NewStoreWithOptions(redis.Gohub_Redis.Client, limiterlib.StoreOptions{
		// 为 limiter 设置前缀，保持 redis 里数据的整洁
		Prefix: configYaml.Gohub_Config.App.Name + ":limiter:",
	})
	if err != nil {
		logger.LogIf(err)
		return context, err
...
// routeToKeyString 辅助方法，将 URL 中的 / 格式为 - , : 格式为 _
