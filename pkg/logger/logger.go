...
func InitLogger(filename string, maxSize, maxBackup, maxAge int, compress bool,
	...
	// 获取日志写入介质
	writeSyncer := getLogWriter(filename, maxSize, maxBackup, maxSize, compress, logType)

	// 设置日志等级, 具体请见 configYaml/log.go 文件
	...
	zap.ReplaceGlobals(Gohub_Logger)
}
...
// getLogWriter 日志记录介质, Gohub 中使用了两种介质, os.Stdout 和 文件
func getLogWriter(filename string, maxSize, maxBackup, maxAge int, compress bool,
	logType string) zapcore.WriteSyncer {
...
func Dump(value interface{}, msg ...string) {
	...
	// 判断第二个参数是否传参 msg
	if len(msg) > 0 {
		Gohub_Logger.Warn("Dump", zap.String(msg[0], valueString))
	} else {
		Gohub_Logger.Warn("Dump", zap.String("data", valueString))
	}
}
...
func LogIf(err error) {
	if err != nil {
		Gohub_Logger.Error("Error Occurred:", zap.Error(err))
	}
}
...
func LogWarnIf(err error) {
	if err != nil {
		Gohub_Logger.Warn("Error Occurred:", zap.Error(err))
	}
}
...
func LogInfoIf(err error) {
	if err != nil {
		Gohub_Logger.Info("Error Occurred:", zap.Error(err))
	}
}
...
func InfoString(moduleName, name, msg string) {
	Gohub_Logger.Info(moduleName, zap.String(name, msg))
}

func WarnString(moduleName, name, msg string) {
	Gohub_Logger.Warn(moduleName, zap.String(name, msg))
}

func ErrorString(moduleName, name, msg string) {
	Gohub_Logger.Error(moduleName, zap.String(name, msg))
}

func FatalString(moduleName, name, msg string) {
	Gohub_Logger.Fatal(moduleName, zap.String(name, msg))
}
...
func InfoJSON(moduleName, name string, value interface{}) {
	Gohub_Logger.Info(moduleName, zap.String(name, jsonString(value)))
}

func WarnJSON(moduleName, name string, value interface{}) {
	Gohub_Logger.Warn(moduleName, zap.String(name, jsonString(value)))
}

func ErrorJSON(moduleName, name string, value interface{}) {
	Gohub_Logger.Error(moduleName, zap.String(name, jsonString(value)))
}

func FatalJSON(moduleName, name string, value interface{}) {
	Gohub_Logger.Fatal(moduleName, zap.String(name, jsonString(value)))
}

func jsonString(value interface{}) string {
	b, err := json.Marshal(value)
	if err != nil {
		Gohub_Logger.Error("Logger", zap.String("JSON marshal error", err.Error()))
	}
	return string(b)
}
